...
The default pattern I kept seeing in small LLM apps is: pick one model, send every query to it. That's simple, and it's also wasteful, because most queries in a typical workload are easy. "What is 2+2" and "explain the trade-offs between microservices and a monolith" are not the same query, but a lot of systems pay GPT-4 prices for both.
